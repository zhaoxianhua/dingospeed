...
// 整个文件
func (d *DownloaderDao) FileDownload(ctx context.Context, hfUri, blobsFile, etag, orgRepo, fileName, dataType, authorization string,
	fileSize, startPos, endPos int64, responseChan chan []byte, isInnerRequest bool) {
	var (
		wg sync.WaitGroup
	)
	...
	tasks := d.constructTask(ctx, dingFile, hfUri, etag, orgRepo, fileName, dataType, authorization,
		startPos, endPos, fileSize, responseChan, isInnerRequest)
	wg.Add(1)
	go func() {
		defer func() {
			wg.Done()
...
func (d *DownloaderDao) constructTask(ctx context.Context, dingFile *downloader.DingCache, hfUri, etag, orgRepo, fileName, dataType, authorization string,
	startPos, endPos, fileSize int64, responseChan chan []byte, isInnerRequest bool) []common.Task {
	var (
		tasks []common.Task
	)
	...
	existPosition, curPos := analysisFilePosition(dingFile, startPos, endPos)
	if !isInnerRequest && config.SysConfig.IsCluster() && !existPosition {
		if response, err := d.getRequestDomainScheduler(dataType, orgRepo, fileName, etag, curPos, endPos, fileSize); err != nil {
			...
			if response.SchedulerType == consts.SchedulerYes {
				...
				if endPos <= response.MaxOffset {
					...
					// 需要重新拆分任务
					taskParam.Domain = speedDomain
					beforeTasks := getContiguousRanges(startPos, response.MaxOffset, taskParam)
