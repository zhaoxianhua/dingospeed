...
func (f *FileDao) FileGetGenerator(c echo.Context, repoType, org, repo, commit, fileName, method string) error {
	...
	if method == consts.RequestTypeHead {
		...
		return f.FileChunkGet(c, hfUrl, blobsFile, filesPath, fileName, authorization, pathInfo.Size, startPos, endPos, respHeaders)
	} else {
...
func (f *FileDao) FileChunkGet(c echo.Context, hfUrl, blobsFile, filesPath, fileName, authorization string, fileSize, startPos, endPos int64, respHeaders map[string]string) error {
	...
	go downloader.FileDownload(ctx, hfUrl, blobsFile, filesPath, fileName, authorization, fileSize, startPos, endPos, responseChan)
